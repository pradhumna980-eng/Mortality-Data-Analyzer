...
class ActuarialAnalyzer:
    ...
    def seasonal_mortality(self):
        ...
        rates = {m: (deaths[m]/exposure[m] if exposure[m] > 0 else 0) for m in exposure}
    ...
    def export_results(self, age_band, disease, outfile="mortality_results.xlsx"):
        ...
        ws1.append([age_band, disease, n, probs.get(1,0), probs.get(2,0), probs.get(3,0)])
        ...
        if n > 0:
            ...
            plt.savefig(os.path.join(static_dir, "conditional_probability.png"))
            plt.close()
        ...
        if rates:
            ...
            plt.savefig(os.path.join(static_dir, "seasonal_mortality.png"))
            plt.close()

        if lifetable:
            plt.figure()
            plt.plot([r["AgeBand"] for r in lifetable], [r["qx"] for r in lifetable])
            plt.xlabel("Age Band")
            plt.ylabel("qx")
            plt.title("Life Table Mortality Rate by Age Band")
            plt.xticks(rotation=45)
            plt.tight_layout()
            plt.savefig(os.path.join(static_dir, "life_table_qx.png"))
            plt.close()
